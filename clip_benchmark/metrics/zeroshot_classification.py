...
def zero_shot_classifier(model, tokenizer, classnames, templates, device, amp=True):
    ...
    with torch.no_grad(), autocast():
        ...
        for classname in tqdm(classnames):
            ...
            if 'clip' in model.__class__.__name__.lower():
                 #### implementation of the original openclip
                inputs = tokenizer(texts).to(device)  # tokenize
                class_embeddings = model.encode_text(inputs)
            else:
                #### implementation of the SAIL
                inputs = tokenizer(texts, padding=True, truncation=True, max_length=1024, return_tensors='pt').to(device)  # tokenize
                class_embeddings = model.encode_text(inputs, text_list = texts)
...
def run_classification(model, classifier, dataloader, device, amp=True):
    ...
    with torch.no_grad():
        ...
        for images, target in tqdm(dataloader):
            if 'clip' not in model.__class__.__name__.lower():
                if isinstance(images, list):
                    # food 101
                    images = torch.stack([img['pixel_values'][0] for img in images])
                else:
